...
*   Stage `Build artifact`: In deze stage bouw je een docker image van de nodeJS applicatie. (Tip: Zorg eerst voor een werkende Dockerfile en test deze lokaal). De image wordt vanuit de pipeline gebuild.
*   Stage `Push artifact`: In deze stage push je de gebouwde image naar dockerhub. Je maakt hier gebruik van een publieke image op het profiel van één van je teamgenoten.
